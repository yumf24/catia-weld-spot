...
### 受控同件 pair 离线研究（永久非生产路径）

固定的跨件生产基线为 TP/FP/FN=`30/6/10`；在相同数据集上无约束开启同件 pair 的离线风险基线为
`40/172/0`。OP08 已复核：在既定跨件边界内，仅靠补回 gap 造成的漏检理论上最多达到 TP=`33`，低于
严格 recall 门槛所需的 TP=`37`，因此该路线不可达，后续仅研究受控同件 pair 的离线策略。

这不是将同件 pair 加到现有基线的增量方案：即使额外恢复 10 个 TP 且不新增 FP，保留 `30/6/10`
也只能得到 TP/FP=`40/6`（precision=`40/46`，约 `86.96%`），仍低于严格 90% 门槛。任何可行策略都
必须以同一套几何门槛同时重筛跨件和同件 pair；它不能改变 `allow_same_part_pairs=false`、生产 CLI、
候选生成或生产默认值。研究输出始终只是 `component-simplify` 的单数据集离线证据，不能据此开启生产
同件行为或宣称跨零件泛化。
