...
| [构建冻结选面模板](#构建冻结选面模板) | `scripts/build_plane_selection_template.py` | 否 | 注册主/参考 STEP | 模板 JSON |
| [按冻结模板选面](#按冻结模板选面) | `scripts/select_template_planes.py` | 否 | 注册主 STEP + 模板 | 受管运行目录内选面/审计 |
| [精确评测冻结选面](#精确评测冻结选面) | `scripts/evaluate_template_plane_selection.py` | 否 | 选面运行 + 参考 STEP | 评测 JSON/报告 |
...
**代码路径**：`scripts/build_plane_selection_template.py`（标签构建及契约校验在
`src/weld_core/plane_reference_labels.py` 与 `plane_selection_template.py`）。

**用途**：一次性将人工 `surface_reference` STEP 的精确 CAD 面标签冻结为模板。构建会核验注册
主/参考 STEP 的 SHA-256，并以法向、平面距离和 OCCT 公共面积/覆盖率建立可审计的一对一标签。
参考 STEP 只允许在此构建和后述评测命令中读取，不能作为日常选面的输入。

```bash
python scripts/build_plane_selection_template.py component-simplify --output templates/component-simplify/plane-selection-template.json
```

可先不写文件地审计标签构建：

```bash
python scripts/build_plane_selection_template.py component-simplify --dry-run
```

## 按冻结模板选面

**代码路径**：`scripts/select_template_planes.py`（核心在
`src/weld_core/template_plane_selection.py`）。

**用途**：日常运行只读取注册主 STEP 和冻结模板，核验主 STEP SHA-256、每个 `part`/稳定 STEP
face index 与边界指纹后，输出单 CAD face 的 `faces.selected.json` 和全量
`selection_audit.json`，并登记到新建的受管运行目录。主 STEP SHA 不一致、index 缺失或指纹不同
会以非零退出，且不产生选面文件；此时必须重建模板，不能通过放宽容差继续运行。

```bash
python scripts/select_template_planes.py component-simplify --run-label template-selection
```

可用 `--template <path>` 指定受控模板的替代位置。运行时不读取 `surface_reference`。

## 精确评测冻结选面

**代码路径**：`scripts/evaluate_template_plane_selection.py`（核心在
`src/weld_core/exact_plane_selection_evaluation.py`）。

**用途**：仅在验收阶段读取人工参考 STEP，以 OCCT 布尔公共面积而非 AABB 对运行目录内的
`faces.selected.json` 计算单 CAD face 的 TP/FP/FN、precision 与 recall。正式通过要求
precision `> 0.90`、recall `> 0.95`，且匹配满足法向 `<=0.5°`、平面距离 `<=0.05mm` 与
source 覆盖率 `>=0.95`。输出 `plane_selection_evaluation.json` 和 Markdown 报告并登记清单。

```bash
python scripts/evaluate_template_plane_selection.py component-simplify --run-dir data/component-simplify/<template-selection-run-id>
```
...
**输入**：通用流程使用 `faces.enriched.json`（`FacesDocument`，只用 `surface_type=="planar"` 且
`manual_review==False` 且 `vertices` 非空的面）。`component-simplify` 的受管模板流程必须使用同一运行
目录中已登记的 `faces.selected.json`；传入 `faces.enriched.json` 或缺失模板追溯信息会失败。
...
component-simplify 冻结模板流程：

```bash
python -m weld_core.pipeline data/component-simplify/<template-selection-run-id>/faces.selected.json data/component-simplify/<template-selection-run-id>/candidates.json
```
