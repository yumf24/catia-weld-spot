...
**做了什么**
- 更新 `docs/ALGORITHM.md`：以双泳道 Mermaid 图准确展示通用 STEP 选面链与 CATIA 面提取/顶点富化链如何汇合到候选生成核心。
- 图中逐节点写明输入/输出文件、核心动作和目的；`candidates.json` 后以虚线标注仅由 `--write` 触发的可选 CATIA 回写。
- 将冗长步骤段落压缩为对应图节点的表格，集中记录阈值、拒绝分支、数据状态和关键代码。
...
## 2026-07-22 16:47:20 +08:00 - 更新候选焊点算法文档

**做了什么**
- 新增 `docs/ALGORITHM.md`，按实际执行顺序说明 STEP 通用选面链、CATIA 提取/STEP 顶点富化支路及其汇合后的配对、区域、布点、过滤和稳定编号。
- 明确当前代码产物为 `faces.general-selected.json`，CATIA 全流程使用 `faces.enriched.json` 直接进入核心；不存在 `faces.selected.json`，且全流程不会在富化后再调用通用选面。
- 文档补充一张与步骤对应的 Mermaid 线性流程图、阈值、分支、拒绝路径和关键函数位置。
