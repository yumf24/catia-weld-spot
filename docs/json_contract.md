...
| `pairs[].exact_region_ref` | str/null | 接受 pair 的 OCCT 精确公共区域 BREP 相对引用；AABB 仅是预筛数据，不能作为区域几何使用。 |

## interface_region_audit.json（精确平面接口区域审计）

由通用选面命令与 `pair_audit.json` 同时生成。每个接受的 pair 都有一条记录，`geometry_ref`
指向运行目录内的 `exact_interface_regions/*.brep`。该 BREP 是将对侧 CAD 面精确投影到
支持平面后由 OCCT 布尔公共形状得到的区域，可保留非矩形边界、孔和凹口；候选布置不得把
投影 AABB 误作该区域。记录还保留两侧 face ID、精确公共面积、双方覆盖率、实际区域有效宽度
及可追溯失败原因。每条记录还保留 `plane_origin` 和 `normal`，作为 BREP
区域二维覆盖布局的固定世界坐标框架；候选布局会逐点以 BREP 精确区域复核。
该产物只依赖登记的 `primary_model`。
