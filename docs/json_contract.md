...
| `pairs[].gap_layer` | `strict`/`extended`/`beyond_extended`/null | 审计层：gap≤0.2 mm 为 `strict`，0.2<gap≤1.5 mm 为 `extended`，更大值为 `beyond_extended`；未量得 gap 时为 null。该标签不改变当前生产阈值。 |
...
该比较仅适用于明确给定的单数据集离线运行，不构成跨零件泛化结论，也不改变生产默认参数。
